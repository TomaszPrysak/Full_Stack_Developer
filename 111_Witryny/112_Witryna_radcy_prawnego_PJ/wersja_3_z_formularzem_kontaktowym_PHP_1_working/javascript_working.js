...
// Formularz
// Wyczyszczenie pól formularza po zamknieciu go.
$('.modal').on('hidden.bs.modal', function (e) {
  var polaFormularza = $(this).find(".form-control");
  for (var i = 0; i < polaFormularza.length; i++) {
    $("#formularz").find(".form-control").eq(i).val("");
  }
})

// Formularz
// Wyczyszczenie pól formularza po kliknięciu przycisku "Wyczyść formularz"
function wyczysc_formularz() {
  var polaFormularza = $("#formularz").find(".form-control");
  for (var i = 0; i < polaFormularza.length; i++) {
    $("#formularz").find(".form-control").eq(i).val("");
  }
};
...
// Formularz
// Zapobieganie przed wyłączaniem formularza i scrollowaniem strony na samą górę
// po naciśnięciu przycisku "Wyślij wiadomość"
$('#wyslanie_formularza_kontaktowego').click(function(e){
    e.preventDefault();
});
